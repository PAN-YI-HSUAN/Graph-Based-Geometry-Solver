...
def get_segmented(data):
    for problem in data:
        problem['segmented_text'] = []
        res = parse(problem['original_text'])
        sent_len = len(res['sentences'])
        for i in range(sent_len):
            for token in res["sentences"][i]["tokens"]:
                problem['segmented_text'].append(token['word'])
    return data
...
pairs_test, generate_nums_test, copy_nums_test = transfer_num_test(data_test)

generate_nums += generate_nums_test
tmp = max(copy_nums, copy_nums_test)
copy_nums = tmp
...
temp_pairs = []
for pair, group_num in zip(pairs_test, group_data_test):
    pair = list(pair)
    pair.append(group_num['group_num'])
    pair = tuple(pair)
